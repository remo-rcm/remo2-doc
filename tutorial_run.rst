...
``ModelRun/REMO_MPI`` should more or less look like this:
...
will execute the run. Go to the ``/build`` directory in which you will
...
``/build`` directory try to type the following:
...
this should be on the WORK partition which you find at ``/work`` on
...
least one project. You can then find you personal WORK directory at:

.. code:: bash

    /work/YOUR_PROJECT_ID/YOUR_USER_ID

| where you should have the tutorial data available. If you run the
...
``/data`` in your ``/ModelRun`` directory which has the following
structure:
...
| This is the main input data REMO requires to start and run a
  simulation. The ``/libs`` directory contains data of the so-called
  ’soil library’. This includes monthly data of the soil albedo,
  vegetation, etc. The second directory ``/xa`` contains the forcing
  data in a 6-hourly resolution. Each file contains all data that is
  required by REMO to setup the *lateral boundary conditions* for a
  certain timestep during the simulation. This is necessary, since REMO
  is a *regional* model. It does not model the entire globe and
  consequently, it will need some boundary conditions derived from a
  global data set. This is exactly what the ``/xa`` directory contains.
  ...
  boundary data from two consecutive forcing files.
| The forcing files (also often called ’a-files’) satisfy a certain
  naming convention which is in this case ``a051000aYYYYMMDDHH`` and
  where ``YYYYMMDDHH`` denotes the date in time of the forcing data in
  ...
  is that REMO also and only reads the *initial conditions* from the
  forcing file when the simulation starts.
...
test\_remo.ksh and you find it in the run directory at:

::

    ModelRun/test_remo.ksh 
...
| Here, we use the environment variable ``$PWD`` since this should
  simply set the directory structure according to your current run
  directory. The ``BASEDIR`` denotes the run directory (e.g, where the
  ``/data`` subdirectory is located) and ``MODELDIR`` is the root
  directory of the REMO Code.
...
If you submitted the batch job script to the MISTRAL scheduler, you can
check your job status using the ``squeue`` command, e.g.
...
You will also find a batch log file (something like ``slurm-....out``)
...
that REMO starts by creating output files in the ``/data/test/``
...
| The naming convention here is ``e031001e_cCCC_YYYYMM`` (“e” =
  ...
  number ``CCC``, see Appendix [app:codelist]). The files are stored in
...
| You can see that this file contains only one field (code 167) of
  ...
  with the naming convention ``e031001n_cCCC_YYYYMM`` (so called
  ...
  Furthermore, the other result directory ``/xt`` is empty as well since
  this will contain REMO data in 6-hourly resolution.
...
| If you look carefully at the run script you will see that this will
  set a parameter ``KSE=744`` which is then fed into the ``NAMELIST``
  ...
  January of 1990 (31 days x 24 hours/day = 744 hours).
...
``tail -n 100 -f slurm-....out`` for continous update of the output),
you can follow the progress in hourly steps,e.g.
...
-  ``e031001fYYYYMMDDHH``: contains restart data (f-file) at timestep
   n-2

-  ``e031001gYYYYMMDDHH``: contains restart data (g-file) at timestep
   n-1

| The m- and s-files contain monthly mean values and standard deviations
...
During the conversion to ``NetCDF``, the file information was connected
...
the other result files and maybe use ``ncview`` (use the command
